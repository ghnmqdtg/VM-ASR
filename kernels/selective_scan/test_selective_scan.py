...
def build_selective_scan_fn(
    selective_scan_cuda: object = None, mode="mamba_ssm", tag=None
):
    MODE = mode
    ...
    class SelectiveScanFn(torch.autograd.Function):
        ...
        def forward(
            ...
            if MODE in ["mamba_ssm"]:
                ...
                out, x, *rest = selective_scan_cuda.fwd(
                    u, delta, A, B, C, D, z, delta_bias, delta_softplus, nrows
                )
        ...
        def backward(ctx, dout, *args):
            ...
            if delta_bias is not None:
                if ddelta_bias.dtype != getattr(
                    ctx, "_delta_bias_dtype", ddelta_bias.dtype
                ):
...
if MODE in ["mamba_ssm"]:
    ...
    selective_scan_fn = build_selective_scan_fn(
        selective_scan_cuda_core, mode="sscorendstate"
    )
    selective_scan_ref = build_selective_scan_fn(selective_scan_cuda, mode="mamba_ssm")
    ...
    selective_scan_fn = build_selective_scan_fn(
        selective_scan_cuda_oflex, mode="ssoflex"
    )
    selective_scan_ref = build_selective_scan_fn(selective_scan_cuda, mode="mamba_ssm")
else:
