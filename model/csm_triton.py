...
def cross_scan_fwd(
    x: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=0
):
    if in_channel_first:
        B, C, H, W = x.shape
        if scans == 0:
...
def cross_merge_fwd(
    y: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=0
):
    if out_channel_first:
        ...
        if scans == 0:
            ...
            y = y[:, 0] + y[:, 1].view(B, -1, W, H).transpose(
                dim0=2, dim1=3
        ...
        if scans == 0:
            ...
            y = y[:, :, 0] + y[:, :, 1].view(B, W, H, -1).transpose(
                dim0=1, dim1=2
        ...
        if scans == 0:
            y = torch.stack(
                [
                    ...
                    torch.flip(
                        x[:, 3].transpose(dim0=2, dim1=3).flatten(2, 3), dims=[-1]
                    ),
                ],
                dim=1,
            )
        ...
        if scans == 0:
            y = torch.stack(
                [
                    ...
                    torch.flip(
                        x[:, :, :, 3].transpose(dim0=1, dim1=2).flatten(1, 2), dims=[1]
                    ),
                ],
                dim=2,
            )
    ...
    def check_csm_triton():
        ...
        if True:
            ...
            res1 = triton.testing.do_bench(
                lambda: cross_scan_fn(x, True, True, False).sum().backward()
            )
            ...
            res4 = triton.testing.do_bench(
                lambda: cross_merge_fn(y, True, True, False).sum().backward()
            )
        ...
        for cs0, cm0, cs1, cm1 in [
            ...
            (
                unidi_scan,
                unidi_merge,
                lambda x: cross_scan_fn(x, scans=1),
                lambda x: cross_merge_fn(x, scans=1),
            ),
            (
                bidi_scan,
                bidi_merge,
                lambda x: cross_scan_fn(x, scans=2),
                lambda x: cross_merge_fn(x, scans=2),
            ),
            ...
            (
                ...
                lambda x: cross_merge_fn(
                    x.permute(0, 3, 4, 1, 2), out_channel_first=False
                ),
            ),
            (
                cross_scan,
                cross_merge,
                lambda x: cross_scan_fn(
                    x.permute(0, 2, 3, 1),
