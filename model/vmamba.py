...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,
        ...
        self.out_norm, forward_type = self.get_outnorm(
            forward_type, self.d_inner, channel_first
        )
        ...
        FORWARD_TYPES = dict(
            ...
            # ===============================
            v051d=partial(
                self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="unidi"
            ),
            v052d=partial(
                self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="bidi"
            ),
            v052dc=partial(
                self.forward_corev2,
                force_fp32=False,
            ...
            v3=partial(
                self.forward_corev2, force_fp32=False, selective_scan_backend="oflex"
            ),
        )
        ...
        **kwargs,
        ...
        if _scan_mode == 3:
            ...
            y_col = (
                scan_rowcol(
                    y_row,
                    proj_weight=self.x_proj_weight.view(4, -1, D)[2:]
                    .contiguous()
                    .to(y_row.dtype),
                    proj_bias=(
                    ...
                    dt_weight=self.dt_projs_weight.view(4, D, -1)[2:]
                    .contiguous()
                    .to(y_row.dtype),
                    dt_bias=(
...
class SS2Dv3:
    def __initxv__(
        ...
        **kwargs,
        ...
        self.out_norm, forward_type = SS2Dv2.get_outnorm(
        ...
        self.dts_dim = dict(
            xv1a=self.dt_rank, xv2a=self.d_inner, xv3a=4 * self.dt_rank
        )[mode]
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        us = cross_scan_fn(
            us.contiguous(), in_channel_first=self.channel_first, out_channel_first=True
        ).view(B, -1, L)
        Bs = cross_scan_fn(
            Bs.contiguous(),
        ...
        if self.dts_dim == self.dt_rank:
            dts = F.conv1d(
                dts,
                self.dt_projs_weight.view(4 * self.d_inner, self.dt_rank, 1),
                None,
                groups=4,
            )
        ...
        y: torch.Tensor = cross_merge_fn(
            ys.contiguous(), in_channel_first=self.channel_first, out_channel_first=True
        )
...
class SS2Dm0:
        ...
        **kwargs,
        ...
        if initialize in ["v1"]:
            ...
            self.Ds = nn.Parameter(
                torch.ones((k_group, dt_rank, int(d_inner // dt_rank)))
            )
            self.A_logs = nn.Parameter(
                torch.randn((k_group, dt_rank))
            )  # A == -A_logs.exp() < 0; # 0 < exp(A * dt) < 1
            self.dt_projs_bias = nn.Parameter(
                0.1 * torch.randn((k_group, dt_rank))
            )  # 0.1 is added in 0430
        elif initialize in ["v2"]:
            # simple init dt_projs, A_logs, Ds
            self.Ds = nn.Parameter(
    ...
    def forward_corem0(
        ...
        **kwargs,
        ...
        if self.initial_state is not None:
            self.initial_state = nn.Parameter(
                final_state.detach().sum(0, keepdim=True), requires_grad=False
            )
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pos_embed = (
            self._pos_embed(dims[0], patch_size, imgsize) if posembed else None
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.pos_embed is not None:
            pos_embed = (
                self.pos_embed.permute(0, 2, 3, 1)
                if not self.channel_first
                else self.pos_embed
            )
            x = x + pos_embed
